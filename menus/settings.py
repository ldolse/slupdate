...
class SettingsMenu(DeclarativeMenuAdapter, BaseMenu):
    def __init__(self):
...
class DatMenu(DeclarativeMenuAdapter, BaseMenu):
    def __init__(self):
        ...
        self.options = [
            ...
            MenuItem(text="c. Back", is_back=True, requires_platform=False),
        ]
    ...
    def _del_datpath_function(platform: Platform) -> None:
        ...
        if answer["dat"] == "back":
            return ResultObject.complete(
                total_processed=0, destination_menu="settings_menu"
            )
        else:
    ...
    @staticmethod
    def _platform_add_dat_function(
        platform: Platform, platform_manager: PlatformManager
    ):
