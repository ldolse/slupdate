...
class CDMedia:
    ...
    def zip_path(self) -> Optional[str]:
        """Get the ZIP path for this media"""
        return self._zip_path

    @zip_path.setter
    def zip_path(self, value: Optional[str]):
        """Set the ZIP path for this media"""
    ...
    def chd_path(self) -> Optional[str]:
        ...
        if (
            self._chd_path is None
            and self.dat_game_entry is not None
            and self.softlist_title is not None
    ...
    def dat_entries(self) -> list["DATGameEntry"]:
        ...
        for entry in self._all_dat_references.values():
            group = (
                entry.dat.dat_group if hasattr(entry.dat, "dat_group") else "unknown"
            )
    ...
    def get_or_create_media(self, game_entry: "DATGameEntry") -> Tuple[CDMedia, bool]:
            ...
            if sig[1] == "SHA1" and sig in self.media_hashes:
                ...
                if matched_cdm is None:  # First match (only CRC available)
                    if existing_media.total_rom_size == size:
                        print(
                            f"Info: CRC only match {game_entry.dat}: {game_entry.name}"
                        )
                        matched_cdm = existing_media
                    else:
                        print(
                            f"⚠️ Size mismatch for CRC-only match - {game_entry.dat}: {game_entry.name}"
                        )
                        return None, False
                elif (
                    matched_cdm is not None and matched_cdm != existing_media
                ):  # Potential collision!
                    if matched_cdm.total_rom_size == existing_media.total_rom_size:
                        print(
                            f"⚠️ Possible duplicate media via CRC + SHA1 hash types for {game_entry.dat}: {game_entry.name}"
                        )
                        print(
                            f"   Matched record: {matched_cdm.dat_game_entry.dat}: {matched_cdm.dat_game_entry.name}"
                        )
                        return None, False
        ...
        if matched_cdm:
            ...
            for sig in [sig for sig in (sha1_sig, crc_sig) if sig]:
                if (
                    sig not in self.media_hashes
                    or self.media_hashes[sig] != matched_cdm
                ):
    ...
    def _calculate_hashes(self, game_entry: "DATGameEntry") -> Tuple[Tuple, Tuple, int]:
        ...
        for rom in game_entry.roms:
            ...
            try:
                ...
                print(
                    f"⚠️   Invalid or missing 'size' attribute in ROM '{rom.name}'. Defaulting to 0."
                )
                total_size += 0
                pass
        ...
        crc_signature = (
            (crc_builder.hexdigest(), "CRC_SHA1") if has_crc and rom_count > 0 else None
        )
