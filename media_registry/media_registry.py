...
class CDMedia:
    ...
    def matched(self) -> bool:
        ...
        if hasattr(self, '_checking_matched') and self._checking_matched:
            return False
        
        self._checking_matched = True
        ...
        try:
            ...
            if (self.dat_game_entry is not None and 
                ...
                # Check if dat_group attribute exists and access it safely
                if hasattr(self.dat_game_entry.dat, 'dat_group'):
                    dat_group = self.dat_game_entry.dat.dat_group
                    if self.softlist_part is not None and dat_group != "MAME-Comment":
                        return True
            return False
        finally:
            self._checking_matched = False
