...
def find_dat_matches(platform: Platform) -> None:
    ...
    for sl_title, sl_data in sl_platform_dict.items():
        for disc, disc_data in sl_data["parts"].items():
            if "source_dat" in disc_data:
                ...
                dat_entry_name = dathash_platform_dict["hashes"][datfile][source_sha][
                    "name"
                ]
                if dat_entry_name in dathash_platform_dict["name_lookup"][datfile]:
                    if (
                        "softlist_matches"
                        not in dathash_platform_dict["name_lookup"][datfile][
                            dat_entry_name
                        ]
                    ):
                        dathash_platform_dict["name_lookup"][datfile][dat_entry_name][
                            "softlist_matches"
                        ] = [(sl_title, disc)]
                    else:
                        dathash_platform_dict["name_lookup"][datfile][dat_entry_name][
                            "softlist_matches"
                        ].append((sl_title, disc))
    ...
    total_source_ref = sum(
        1
        for softlist_entry in sl_platform_dict.values()
        for part in softlist_entry["parts"].values()
        if "source_sha" in part
    )
    # Count the number of entries where 'source_found' is True
    total_source_found = sum(
        1
        for softlist_entry in sl_platform_dict.values()
        if softlist_entry["source_found"]
    )
    ...
    chd_count = sum(
        ...
        for softlist_entry in sl_platform_dict.values()
        for part in softlist_entry["parts"].values()
        if "chd_found" in part and part["chd_found"]
    )
    # Count the total number of 'parts' across all entries
    total_parts = sum(
        len(softlist_entry["parts"]) for softlist_entry in sl_platform_dict.values()
    )
    # Count the number of parts that have a 'source_dat' entry
    total_source_dat = sum(
        1
        for softlist_entry in sl_platform_dict.values()
        for part in softlist_entry["parts"].values()
    ...
    total_source_rom = len(
        list(
            part["source_rom"]
            for softlist_entry in sl_platform_dict.values()
            for part in softlist_entry["parts"].values()
            if "source_rom" in part
        )
    )
    ...
    print(
        f"  {total_source_found} / {total_softlist_entries} Software List Entries have DAT matches"
    )
