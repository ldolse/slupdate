...
class Sector:
    SYNC_MARK = bytes([0x00, 0xFF, 0xFF, 0xFF, 0xFF, 0xFF, 0xFF, 0xFF, 0xFF, 0xFF, 0xFF, 0x00])

    SCRAMBLE_TABLE = array.array('B', [0x00, 0x00, 0x00, 0x00, 0x00, 0x00, 0x00, 0x00, 0x00, 0x00, 0x00, 0x00, 0x01, 0x80, 0x00, 0x60, 0x00, 0x28,
    ...
    def scramble(sector):
        ...
        sync = sector[:12]
        if sync != Sector.SYNC_MARK:
            return sector

        scrambled = bytearray(len(sector))
        for i in range(2352):
            scrambled[i] = sector[i] ^ Sector.SCRAMBLE_TABLE[i]

        if len(sector) > 2352:
            scrambled[2352:] = sector[2352:]

        return bytes(scrambled)

    @staticmethod
    def get_user_data(data, interleaved=False, file_number=0):
        if len(data) == 2352:
            ...
            if data[15] == 0:
                ...
                return Sector.get_user_data_from_mode2(data, interleaved, file_number)
            else:
                return data
        elif len(data) == 2336:
            return Sector.get_user_data_from_mode2(data, interleaved, file_number)
    ...
    @staticmethod
    def get_user_data_from_mode2(data, interleaved=False, file_number=0):
        if len(data) != 2352 and len(data) != 2336:
            return data

        pos = 0
        if len(data) == 2352:
            if data[:12] != Sector.SYNC_MARK or data[15] != 0x02:
                return data
            pos = 16

        if interleaved and data[pos] != file_number:
            return None

        length = 2324 if (data[pos + 2] & 0x20) == 0x20 else 2048
        pos += 8
    ...
    def prettify(buffer):
        ...
        if buffer[:12] != Sector.SYNC_MARK:
            return "CD sector"

        result = ["CD-ROM sector"]
        ...
        lba += ((min_val >> 4) * 10 + (min_val & 0x0F)) * 75 * 60
        ...
        result.append(f"Position: {min_val:02X}:{sec:02X}:{frame:02X} (LBA: {lba})")
        ...
        if mode == 0:
            ...
            result.append("Mode 2")
        else:
            result.append("Invalid mode 3")

        block_type = (buffer[15] & 0xE0) >> 5
        block_types = [
            "User data block",
            "Fourth run-in block",
            "Third run-in block",
        ...
        result.append(block_types[block_type])

        status, correct_ecc_p, correct_ecc_q, correct_edc = CdChecksums.check_cd_sector_with_details(buffer)

        if mode == 0:
            if status:
                result.append("Correct sector contents")
            else:
                result.append("Incorrect sector contents")
        elif mode == 1:
            if correct_edc:
            ...
            if all(b == 0 for b in buffer[2068:2076]):
                ...
                result.append("Incorrect zero fill")
        elif mode == 2:
            ...
            if buffer[18] & 0x40:
                result.append("Real-time block")
            result.append("Form 2" if buffer[18] & 0x20 else "Form 1")
            if buffer[18] & 0x10:
                result.append("Trigger block")
            if buffer[18] & 0x08:
                result.append("Data block")
            if buffer[18] & 0x04:
                result.append("Audio block")
            if buffer[18] & 0x02:
                result.append("Video block")
            if buffer[18] & 0x01:
                result.append("End of record")
